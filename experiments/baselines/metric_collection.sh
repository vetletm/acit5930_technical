...
touch $csv_filename
echo "timestamp,pid1,cpu1,mem1,pid2,cpu2,mem2,disk_transfer_ops" >> $csv_filename

disk_io_previous=$(iostat -d sda | tail -n 2 | xargs | awk '{print $2}')

while true
do
  timestamp=$(date +"%Y%m%d-%H%M%S")
  cpu_mem_usage=$(pgrep simple_switch | xargs -I % top -b -n 1 -p % | grep simple_switch | awk '{print $1 "," $9 "," $10}' | tr -d '\n')
  disk_io_current=$(iostat -d sda | tail -n 2 | xargs | awk '{print $2}')
